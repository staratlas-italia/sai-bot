...
var bigquery$1 = require('@google-cloud/bigquery');
var discord = require('discord.js');
require('dotenv/config');

function _interopDefaultLegacy (e) { return e && typeof e === 'object' && 'default' in e ? e : { 'default': e }; }

var discord__default = /*#__PURE__*/_interopDefaultLegacy(discord);

const options = {
    keyFilename: "./service_account.json",
    projectId: "fleetsnapshots",
};
const bigquery = new bigquery$1.BigQuery(options);
const discordClient = new discord__default["default"].Client({
    intents: [
        discord__default["default"].Intents.FLAGS.GUILDS,
        discord__default["default"].Intents.FLAGS.GUILD_MEMBERS,
        discord__default["default"].Intents.FLAGS.GUILD_MESSAGES,
    ],
});
...
discordClient.on("ready", async () => {
    ...
    const guild = (await discordClient.guilds.cache.get(discordGuildId)) ||
        ...
        options: [
            {
                name: "status",
                description: "on/off",
                type: discord__default["default"].Constants.ApplicationCommandOptionTypes.STRING,
                required: true,
            },
        ],
    });
...
discordClient.on("interactionCreate", async (interaction) => {
    ...
    if (commandName === "push") {
        const queryUsers = `SELECT public_key, discord_id, notifications
        ...
        if (options.getString("status") == "on") {
            const member = rows.find((row) => row.discord_id
                .toLowerCase()
            ...
            if (!member.notifications || member.notifications == null) {
                const queryUpdateOn = `UPDATE fleetsnapshots.star_atlas.players
        ...
        // OFF
            ...
            if (member.notifications) {
                const queryUpdateOff = `UPDATE fleetsnapshots.star_atlas.players
        ...
        // ERROR
            ...
            interaction.reply({
                content: "Il comando non è valido, lo status può essere solo on oppure off",
                ephemeral: true,
            });
        }
    }
